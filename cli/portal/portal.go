...
func (s *Server) handleConnect(w http.ResponseWriter, r *http.Request) {
	...
	// Hand the credentials to main and return immediately. Main will tear down
	// the hotspot (dropping this phone's link) and then join the network, so we
	// must respond before the link is lost.
	select {
	case s.provisioned <- Credentials(req):
	default:
	}
