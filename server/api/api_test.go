...
func TestTranslatorRoleRestrictions(t *testing.T) {
	...
	// Translator CAN list ABCs (read-only, for choosing booth monitors)
	...
	// Translator CANNOT create ABCs (admin only)
	req, _ = http.NewRequest(http.MethodPost, ts.URL+"/api/abcs", strings.NewReader(`{"name":"Nope"}`))
	req.Header.Set("Authorization", "Bearer "+token)
	req.Header.Set("Content-Type", "application/json")
	resp, err = http.DefaultClient.Do(req)
	require.NoError(t, err)
